...
async def cmd_portfoy_sil(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        avg_cost = float(mevcut["average_cost_usd"])  # USD birim maliyet
        ...
        # TRY varlıklar için fiyatları normalize et
        if currency == "TRY":
            satis_fiyat_try = satis_fiyat          # kullanıcı TRY girdi
            satis_fiyat_usd = satis_fiyat / usd_try
            avg_cost_try    = avg_cost * usd_try    # gösterim için
        else:
            satis_fiyat_try = satis_fiyat * usd_try
            satis_fiyat_usd = satis_fiyat
            avg_cost_try    = avg_cost * usd_try

        # Kar/Zarar — TRY varlıklar için TRY bazında hesapla
        if currency == "TRY":
            maliyet_toplam_try = adet * avg_cost_try
            satis_toplam_try   = adet * satis_fiyat_try
            kar_zarar_try      = satis_toplam_try - maliyet_toplam_try
            kar_zarar_usd      = kar_zarar_try / usd_try
            kar_zarar_pct      = kar_zarar_try / maliyet_toplam_try * 100 if maliyet_toplam_try > 0 else 0
        else:
            maliyet_toplam_usd = adet * avg_cost
            satis_toplam_usd   = adet * satis_fiyat_usd
            kar_zarar_usd      = satis_toplam_usd - maliyet_toplam_usd
            kar_zarar_try      = kar_zarar_usd * usd_try
            kar_zarar_pct      = kar_zarar_usd / maliyet_toplam_usd * 100 if maliyet_toplam_usd > 0 else 0

        kar_emoji = "🟢" if kar_zarar_usd >= 0 else "🔴"
        kar_sign  = "+" if kar_zarar_usd >= 0 else ""

        # Supabase'e satış yaz — her zaman USD normalize edilmiş fiyatla
        def _full_sell():
            with SessionLocal() as db:
                txn, pnl = crud.sell_asset(
                    ...
                    symbol       = ticker,
                    quantity     = adet,
                    price        = satis_fiyat_usd,
                    currency     = "USD",
                    usd_try_rate = 1.0,
                )
                para_birimi = "TRY" if currency == "TRY" else "USD"
                gosterim_fiyat = satis_fiyat_try if currency == "TRY" else satis_fiyat_usd
                crud.log_event(
                    db           = db,
                    source       = "TELEGRAM_BOT",
                    event_type   = "PORTFOLIO_SELL",
                    message      = (
                        f"TAM SATIŞ: {adet:g}x {ticker} @ "
                        f"{'₺' if currency == 'TRY' else '$'}{gosterim_fiyat:,.4f} | "
                        f"P&L: {kar_zarar_try:+,.2f} TL / {kar_zarar_usd:+,.2f} USD"
                    ),
                    severity     = "INFO",
                    asset_symbol = ticker,
                    metric_value = kar_zarar_usd,
                )
                return pnl

        await loop.run_in_executor(None, _full_sell)

        # Gösterim — TRY varlıklar için TRY fiyat göster
        if currency == "TRY":
            mesaj = (
                f"{kar_emoji} <b>{ticker} — TAM SATIŞ</b>\n\n"
                f"📊 <b>Satış Özeti:</b>\n"
                f"  Adet:          {adet:,g}\n"
                f"  Alış fiyatı:   ₺{avg_cost_try:,.4f}\n"
                f"  Satış fiyatı:  ₺{satis_fiyat_try:,.4f}\n\n"
                f"💰 <b>Gerçekleşen Kar/Zarar:</b>\n"
                f"  <b>{kar_sign}₺{abs(kar_zarar_try):,.2f} TL ({kar_sign}{kar_zarar_pct:.1f}%)</b>\n"
                f"  USD karşılığı: {kar_sign}${abs(kar_zarar_usd):,.2f}\n\n"
            ...
            mesaj = (
                ...
                f"  Satış fiyatı:  ${satis_fiyat_usd:,.2f}\n\n"
                f"💰 <b>Gerçekleşen Kar/Zarar:</b>\n"
                f"  <b>{kar_sign}${abs(kar_zarar_usd):,.2f} USD ({kar_sign}{kar_zarar_pct:.1f}%)</b>\n"
                f"  TL karşılığı: {kar_sign}₺{abs(kar_zarar_try):,.0f}\n\n"
                f"✅ {ticker} portföyden çıkarıldı."
            )
...
async def cmd_portfoy_guncelle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    Pozisyon güncelle veya P&L düzelt.

    Kullanım:
      /guncelle <TICKER> <ADET> <BİRİM_MALİYET>   — adet ve maliyet güncelle
      /guncelle PNL <TICKER> <PNL_USD>             — realized P&L manuel düzelt

    Örnekler:
      /guncelle AVGO 2.767 335
      /guncelle PNL NNF -421.37
    """
    args = ctx.args
    if not args:
        await update.message.reply_text(
            "📝 <b>Kullanım:</b>\n\n"
            "<b>Pozisyon güncelle:</b>\n"
            "/guncelle TICKER ADET BİRİM_MALİYET\n"
            "Örnek: /guncelle AVGO 2.767 335\n\n"
            "<b>P&L düzelt:</b>\n"
            "/guncelle PNL TICKER PNL_USD\n"
            "Örnek: /guncelle PNL NNF -421.37\n"
            "<i>(Negatif = zarar, pozitif = kar)</i>",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    if args[0].upper() == "PNL":
        if len(args) < 3:
            await update.message.reply_text(
                "📝 <b>Kullanım:</b> /guncelle PNL TICKER PNL_USD\n"
                "Örnek: /guncelle PNL NNF -421.37",
                parse_mode=ParseMode.HTML,
            )
            return
        ...
        ticker  = args[1].upper()
        try:
            pnl_usd = float(args[2].replace("$", "").replace(",", ""))
        except ValueError:
            await update.message.reply_text("❌ Geçersiz P&L değeri. Örnek: -421.37")
            return
        ...
        try:
            ...
            def _update_pnl():
                with SessionLocal() as db:
                    ...
                    eski_pnl = pos.realized_pnl_usd
                    pos.realized_pnl_usd = pnl_usd
                    db.commit()
                    crud.log_event(
                        db=db, source="TELEGRAM_BOT", event_type="PNL_CORRECTION",
                        message=f"P&L DÜZELTMESİ: {ticker} | Eski: ${eski_pnl:+.2f} → Yeni: ${pnl_usd:+.2f}",
                        severity="INFO", asset_symbol=ticker, metric_value=pnl_usd,
                    )
                    return eski_pnl

            loop    = asyncio.get_running_loop()
            eski    = await loop.run_in_executor(None, _update_pnl)
            pnl_tl  = pnl_usd * usd_try
            emoji   = "🟢" if pnl_usd >= 0 else "🔴"
            sign    = "+" if pnl_usd >= 0 else ""

            await update.message.reply_text(
                f"{emoji} <b>{ticker} — P&L Düzeltildi</b>\n\n"
                f"  Eski P&L: ${eski:+.2f} USD\n"
                f"  Yeni P&L: <b>{sign}${pnl_usd:,.2f} USD</b>\n"
                f"  TL karşılığı: {sign}₺{abs(pnl_tl):,.2f}",
                parse_mode=ParseMode.HTML,
            )
    ...
    # ── Normal güncelleme modu ────────────────────────────────────────────
    if len(args) < 3:
        await update.message.reply_text(
            "📝 <b>Kullanım:</b> /guncelle TICKER ADET BİRİM_MALİYET\n"
            "Örnek: /guncelle AVGO 2.767 335",
            parse_mode=ParseMode.HTML,
        )
        return
...
async def cmd_haber(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    Kullanım:
      /haber                    — Tam sabah brifingı (makro + portföy)
      /haber macro              — Gerçek makro haberler (VIX, DXY, Fed, Powell)
      /haber us_equity          — ABD ekonomi haberleri (ISM, PMI, NFP, CPI)
      /haber turkiye            — Türkiye ekonomi haberleri (TCMB, faiz, TÜFE)
      /haber AAPL us_equity     — Tek hisse haberleri
      /haber BTC crypto         — Tek kripto haberleri
      /haber XAU commodity      — Emtia haberleri
      /haber IIH tr_fund        — TEFAS fon haberleri

    KURAL: Sembol girilirse tip de girilmek ZORUNLUDUR.
    """
    args = ctx.args

    from data.news_client import (
        get_true_macro_news, get_us_economy_news,
        get_turkey_economy_news, get_asset_news,
        get_daily_news_briefing,
    )

    def _format_news(news_list, baslik):
        """Haber listesini okunabilir formata çevir."""
        if not news_list:
            return f"📭 <b>{baslik}</b>\nSon 30 güne ait kriterlere uygun haber bulunamadı."
        lines = [f"📰 <b>{baslik}</b>", "━" * 28, ""]
    ...
    # ── Tek varlık modu: /haber AAPL us_equity ────────────────────────────
    if len(args) >= 2:
        symbol     = args[0].upper()
        tip_raw    = args[1].lower()
        asset_type = ASSET_TYPE_MAP.get(tip_raw)

        if not asset_type:
            await update.message.reply_text(
                f"⚠️ Geçersiz tip: <b>{tip_raw}</b>\n\n"
                "Geçerli tipler:\n"
                "  us_equity | crypto | commodity | tr_fund",
                parse_mode=ParseMode.HTML,
            )
            return

        await update.message.reply_text(f"⏳ {symbol} ({tip_raw}) haberleri çekiliyor...")
        news = await loop.run_in_executor(None, get_asset_news, symbol, asset_type)
        mesaj = _format_news(news, f"{symbol} Haberleri")
        for chunk in [mesaj[i:i+4000] for i in range(0, len(mesaj), 4000)]:
            await update.message.reply_text(chunk, parse_mode=ParseMode.HTML)
        return

    # ── Tek argüman: kategori komutu ─────────────────────────────────────
    if len(args) == 1:
        kategori = args[0].lower()

        # Sembol girilmiş ama tip girilmemiş
        if kategori not in ("macro", "us_equity", "turkiye", "türkiye",
                            "hisse", "crypto", "kripto"):
            await update.message.reply_text(
                f"⚠️ Varlık tipi belirtilmedi.\n\n"
                f"<b>Kullanım:</b>\n"
                f"  /haber {kategori.upper()} us_equity\n"
                f"  /haber {kategori.upper()} crypto\n"
                f"  /haber {kategori.upper()} commodity\n"
                f"  /haber {kategori.upper()} tr_fund",
                parse_mode=ParseMode.HTML,
            )
            return
        ...
        # Kategori haberleri
        kategori_map = {
            "macro":    (get_true_macro_news,      "🌍 Gerçek Makro Haberler"),
            "us_equity":(get_us_economy_news,      "🇺🇸 ABD Ekonomi Haberleri"),
            "turkiye":  (get_turkey_economy_news,  "🇹🇷 Türkiye Ekonomi Haberleri"),
            "türkiye":  (get_turkey_economy_news,  "🇹🇷 Türkiye Ekonomi Haberleri"),
        }

        if kategori in kategori_map:
            fn, baslik = kategori_map[kategori]
            await update.message.reply_text(f"⏳ {baslik} yükleniyor...")
            news  = await loop.run_in_executor(None, fn)
            mesaj = _format_news(news, baslik)
            for chunk in [mesaj[i:i+4000] for i in range(0, len(mesaj), 4000)]:
                await update.message.reply_text(chunk, parse_mode=ParseMode.HTML)
            return

    # ── Argümansız: Tam sabah brifingı ────────────────────────────────────
    await update.message.reply_text(
        "⏳ Sabah brifingı hazırlanıyor...\n"
        "<i>Makro + portföy haberleri toplanıyor</i>",
        parse_mode=ParseMode.HTML,
    )
    ...
    # Makro haberler
    for key, baslik in [
        ("MAKRO_GERCEK", "🌍 Gerçek Makro"),
        ("ABD_EKONOMI",  "🇺🇸 ABD Ekonomi"),
        ("TURKIYE",      "🇹🇷 Türkiye"),
    ]:
        news  = briefing.get(key, [])
        mesaj = _format_news(news, baslik)
        for chunk in [mesaj[i:i+4000] for i in range(0, len(mesaj), 4000)]:
            await update.message.reply_text(chunk, parse_mode=ParseMode.HTML)

    # Portföy özel — özet format
    portfoy = briefing.get("PORTFOY_OZEL", {})
    if portfoy:
        lines = ["💼 <b>Portföy — Varlığa Özel Haberler</b>", "━" * 28, ""]
        for symbol, news_list in list(portfoy.items())[:10]:
            if not news_list:
                continue
